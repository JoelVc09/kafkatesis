...
# Guarda más datos para poder desplazarse con el rangeslider.
data_buffer = deque(maxlen=300)

VISIBLE_POINTS = 120
GRAPH_MIN_WIDTH = "1700px"

# Cantidad de puntos visibles en la ventana principal.
VISIBLE_POINTS = 120
...
consumer = KafkaConsumer(
    ...
    group_id="dashboard-process-sensors-v2"
)
...
def parse_timestamp(value):
    """
    Convierte el timestamp del mensaje Kafka a datetime.
    Esto evita problemas de escala temporal en Plotly.
    """
...
def get_x_values(data):
    """
    Obtiene los timestamps como datetime.
    """
    return [parse_timestamp(d["process_timestamp"]) for d in data]


def get_visible_range(x_values):
    """
    Define una ventana móvil visible.
    El resto queda disponible en el rangeslider.
    """
    if not x_values:
        return None

    if len(x_values) <= VISIBLE_POINTS:
        return [x_values[0], x_values[-1]]

    return [x_values[-VISIBLE_POINTS], x_values[-1]]


def xaxis_config(x_values):
    """
    Configuración común del eje X con scroll horizontal.
    """
    config = {
        "title": "Tiempo",
        "gridcolor": GRID,
    ...
    visible_range = get_visible_range(x_values)

    if visible_range:
        config["range"] = visible_range
...
def base_layout(title, ytitle, x_values):
    return dict(
        title=title,
        paper_bgcolor=PANEL,
        plot_bgcolor=PANEL,
        font=dict(color=TEXT),
        xaxis=xaxis_config(x_values),
...
def add_batch_regions(fig, data):
    ...
    Solo trabaja con la ventana visible, no con todo el histórico.
    """
...
def update_dashboard(n):
    ...
    if not data_buffer:
        empty = go.Figure()
        empty.update_layout(
            title="Esperando datos desde Kafka...",
            paper_bgcolor=PANEL,
            plot_bgcolor=PANEL,
            font=dict(color=TEXT),
    ...
    all_data = list(data_buffer)

    # Solo se grafica una ventana móvil.
    # Esto evita que los gráficos se compriman o queden en blanco.
    data = all_data[-VISIBLE_POINTS:]
    ...
    ph_status, ph_color = status_by_limits(
        float(latest["ore_pulp_ph"]),
        LIMITS["ore_pulp_ph"]
    )

    density_status, density_color = status_by_limits(
        float(latest["ore_pulp_density"]),
        LIMITS["ore_pulp_density"]
    )
    ...
    fig_air.update_layout(base_layout("Flujo de aire por celda", "Nm³/h", x_values))
    ...
    fig_level.update_layout(base_layout("Nivel de espuma por celda", "mm", x_values))
